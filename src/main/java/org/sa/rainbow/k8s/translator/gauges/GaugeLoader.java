...
public class GaugeLoader implements IGaugeLoader {
  ...
  private final Logger logger = LoggerFactory.getLogger(GaugeLoader.class);

  public void load(Map<String, Map<String, Map>> gaugeSpecMap) {
...
//    return gd;
  ...
  public GaugeDescription load() {
    ...
    var gaugeSpecMap = new HashMap<String, Map<String, Map>>();
    ...
    if (instanceMap != null) {
      for (Map.Entry<String, Map> instSpec : instanceMap.entrySet ()) {
        ...
        if (gaugeTypeSpec == null) {
          logger.error (MessageFormat.format("Cannot find gauge type: {0} referred to in gauge ''{1}''.", gaugeType, gaugeName));
          continue;
        }
